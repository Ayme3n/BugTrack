...
/**
 * Edit Payload Page
 * Form to update an existing payload
 */
...
export default function EditPayloadPage({ 
  params 
}: { 
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        setPayloadId(resolvedParams.id);
        ...
        setError(err.message || 'Failed to fetch payload');
        setLoading(false);
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'Failed to update payload');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    return (
      <div className="max-w-4xl mx-auto">
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-2">Loading payload...</p>
        </div>
      </div>
    );
  }
